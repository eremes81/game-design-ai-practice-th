...
ดังนั้นกระดูกสันหลังของบทนี้จึงเรียบง่าย รักษาแกนให้เป็นโค้ดจนถึงที่สุด แล้วประกบ AI ไว้ที่ห้าจุดของทางเข้า·ทางออก โดยเริ่มจากฝั่งทางออกที่ใช้แรงมือมากที่สุดก่อน — งานตามหาบรรทัดแปลก ๆ จากผลลัพธ์ 1,200 แมตช์ แล้วตั้งสมมติฐาน — แล้วค่อยทำให้เป็นอัตโนมัติ
...
def find_outliers(results, threshold=2.5):
    ...
    means, stds = compute_per_metric(results)   # ค่าเฉลี่ย·ส่วนเบี่ยงเบนมาตรฐานต่อตัวชี้วัด
    outliers = []
    for r in results:
...
รูปแบบการตอบ: ลิสต์มีหมายเลข ไม่มีคำนำ·คำลงท้าย มีแต่สมมติฐาน
```
...
บันทึกเซสชันจริงข้างต้นที่จริงแล้วเป็นการเหยียบสามในห้าตำแหน่ง (ตรวจจับความผิดปกติ·ค้นหาการเปลี่ยน·ตีความความผิดปกติ) พร้อมกัน เมื่อคลี่ห้าตำแหน่งออกเป็นวงจรก็จะหมุนแบบนี้
...
**ตำแหน่ง 4 — ตีความรูปแบบผิดปกติ** ขั้นที่ 2\~3 ข้างต้นคือสิ่งนี้ LLM แปะสมมติฐาน 3\~5 ข้อให้กับ outlier ที่ z-score คัดมา การห้ามวินิจฉัยฟันธงคือเส้นชีวิตของตำแหน่งนี้
...
<svg viewBox="0 0 720 330" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  ...
  <text x="520" y="163" text-anchor="middle" fill="#14532d" font-size="10">ตีความผิดปกติ ②</text>
...
ตัวเลขในวงกลม (①\~⑤) คือลำดับการนำมาใช้ **ตำแหน่ง 3 (รายงาน)** และ **ตำแหน่ง 4 (ตีความความผิดปกติ)** อยู่ขวาบน — จุดที่ ROI (Return on Investment, ผลตอบแทนเทียบกับการลงทุน) สูงและความเสี่ยงต่ำ — จึงเปิดก่อน เพียงเดินสองตัวนี้ ปริมาณงานที่ประมวลได้ก็เพิ่มขึ้น 2\~3 เท่า และผลของการนำมาใช้กว่า 70% ก็ถูกเก็บกลับมาจากตรงนี้ **ตำแหน่ง 2 (เสนอการเปลี่ยน)** อยู่จุดสีแดงขวาล่าง ตัวเลือกที่ผิดอาจกินเวลาตรวจสอบ จึงต้องเปิดอย่างระมัดระวังที่สุดเป็นลำดับสุดท้าย ทุกทีมก็ไม่จำเป็นต้องเปิดครบทั้งห้า — เพียงตำแหน่ง 3·4 ก็ทำให้วันหนึ่งของคนทำบาลานซ์คนเดียวเปลี่ยนไปแล้ว

ความรู้สึกตามจริงของระยะเวลาในการนำมาใช้เป็นแบบนี้ (เป็นการประมาณของผู้เขียน ยังไม่ได้ตรวจสอบ — แปรผันอย่างมากตามขนาดทีม·ความสุกของเครื่องมือ) ตำแหน่ง 3 ใช้ 1\~2 สัปดาห์, เพิ่มตำแหน่ง 4 อีก 2 สัปดาห์, เพิ่มตำแหน่ง 1 อีกหนึ่งเดือน, เพิ่มตำแหน่ง 5 อีก 2 สัปดาห์, ส่วนตำแหน่ง 2 อยู่ท้ายสุด 1\~2 เดือน นี่คืออีกสำนวนหนึ่งของคำที่ว่าอย่าเปิดทั้งหมดในคราวเดียว
...
- **มอบหมายการจำลองเชิงกำหนดให้ LLM** → การจำลองคือโค้ด LLM อยู่แค่ทางเข้า·ทางออก หากความเป็นเชิงไม่กำหนดรั่วเข้าแกน เครื่องมือจะตาย
...
- เปิดจากรายงาน·การตีความความผิดปกติที่ ROI สูงก่อน ส่วนข้อเสนอการเปลี่ยนเปิดอย่างระมัดระวังที่สุดเป็นลำดับสุดท้าย
