...
ประเด็นสำคัญคือ **ยิ่งพิกัดเลื่อนลงล่างมากเท่าไร สัดส่วนที่ AI รับผิดชอบทั้งหมดก็ยิ่งมากขึ้น** "จะตั้งคลาสไว้กี่คลาส" ของ L1 เป็นเรื่องเอกลักษณ์เกม คนจึงต้องถือไว้ ในทางกลับกัน "external key ทั้ง 312 แถวตรงกันหมดไหม" ของ L3 เป็นรูปแบบตายตัวและซ้ำ ๆ AI จึงควรรับไปทั้งก้อน ส่วน L2 อยู่ตรงกลาง — การตัดสินใจเป็นของคน แต่ AI ช่วยหนุนด้วยการดึงขอบเขตผลกระทบว่า "ถ้าเปลี่ยนกฎนี้แล้วจะกระเทือนไปถึงไหน"
...
การป้อนข้อมูลเป็นไปตามหลัก `schema-first` กล่าวคือคนเป็นผู้นิยาม **ชีตข้อมูลจำเพาะ (명세서)** ก่อน จากนั้น Enum และ `.proto` จึงออกมา (VBA (ภาษามาโครของ Excel) Export) แล้วสุดท้ายข้อมูล csv จึงถูกกรอกเข้าไป ดังนั้นพรอมต์ตรวจสอบจึงเริ่มด้วย "อ่านสคีมาก่อน" และตอกย้ำว่าฉบับจริงไม่ใช่เอกสารสคีมา แต่เป็น **JSON ที่ export ออกมาจริง** (`json_over_schema_doc_as_source_of_truth`) เพราะเอกสารโกหกได้ แต่ JSON ที่ export ออกมาโกหกไม่ได้
...
> (A) `effect_id` ทุกตัวใน `skill.json` มีอยู่ใน `effect.json` หรือไม่ (ความสอดคล้องของ external key)
...
> **(A) ความสอดคล้องของ external key — ละเมิด 1 รายการ**
...
`gen_relation_map.py` วิเคราะห์ชื่อคอลัมน์·ค่าของชีตเพื่อตรวจจับความสัมพันธ์ของ external key โดยอัตโนมัติ แล้วดึงออกมาเป็นแผนผังความสัมพันธ์ HTML แบบอินเทอร์แอ็กทีฟ ถ้าใน 3.1.4 คนเป็นผู้วาดลูกศร `skill.effect_id → effect.effect_id` ไว้ในหัว สคริปต์นี้ก็จะวาดลูกศรนั้นออกมาเป็นภาพสำหรับชีตทั้งหมด ตำแหน่งที่การพึ่งพิงไหลย้อนทาง (ความเสี่ยงที่ข้อมูล L3 ไปอ้างถึงโครงร่าง L1 แบบย้อนกลับ) จะกระโดดเด่นออกมาจากภาพในทันที
...
- **การตรวจสอบความสอดคล้อง**: การเทียบ external key ระหว่างสกิลกับเอฟเฟกต์ 312 แถวด้วยมือ เร็วที่สุดก็สองชั่วโมง (วันพฤหัสบดีที่ต้นบทนั้นแหละ) ส่วนการตรวจสอบด้วย AI นั้น ถ้าหักเวลาเตรียมอินพุตออก รายการที่ละเมิดก็ออกมาภายในไม่กี่นาที สิ่งที่ลดลงคือแรงงานในการกระโดด·เทียบ ไม่ใช่การตัดสิน
...
**setup.** export ชีตสองชีต (เช่น สกิล, เอฟเฟกต์) เป็น csv ถ้าทำได้ก็แปลงเก็บไว้เป็น JSON (หลักการที่ว่าผลลัพธ์ที่ export ออกมาคือฉบับจริง ไม่ใช่เอกสาร) เลือก external key หนึ่งคู่ระหว่างสองชีต (เช่น `skill.effect_id → effect.effect_id`)
